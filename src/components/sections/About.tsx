export default function About() {
  return (
    <section id="about" className="py-20 border-b border-gray-100">
      <h2 className="text-xs font-semibold text-gray-400 tracking-widest uppercase mb-8">
        About
      </h2>
      <div className="max-w-2xl space-y-4 text-gray-600 leading-relaxed">
        <p>
          Hi, I'm Ethan. I'm a second-year at Columbia studying Computer Science and Financial
          Economics, originally from Singapore.
        </p>
        <p>
          Before college, I served two years in the Singapore Armed Forces as a lieutenant,
          leading a platoon of around 40 soldiers. That experience shaped how I think about
          communication, ownership, and working under pressure.
        </p>
        <p>
          Since then, I've worked across both technical and financial roles — building Python
          automation tools at a software company, interning at a boutique investment bank, and
          most recently doing equity research at a hedge fund where I build financial models,
          write investment memos, and automate parts of the workflow in Python.
        </p>
        <p>
          I'm drawn to roles where technical ability and business judgment overlap. Outside of
          work, I follow Formula One, play tennis, and play chess.
        </p>
      </div>
    </section>
  )
}
